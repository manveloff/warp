...
//---------------------//
  ...
  protected function get_module_name() {
    ...
    $result = mb_strtolower($this->output->choice("Module where is the middleware to remove", $modules));
  ...
  protected function get_mw_name() {
    ...
    // 2. Get array of all middleware in module $this->module_name
    $middleware = collect($fs->files('warp/modules/'.$this->module_name.'/other/middleware'))->map(function($element){
      return basename($element);
    })->toArray();

    // 3. Ask user about middleware name
    $result = $this->output->choice("Middleware to remove", $middleware);
