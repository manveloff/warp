...
//---------//
  ...
  public function remove() { try {
    ...
    // 2. Remove middleware file
    if($fs->exists('warp/modules/'.$this->resource->module_name.'/other/middleware/'.$this->resource->mw_name)) {
      if(!$fs->delete('warp/modules/'.$this->resource->module_name.'/other/middleware/'.$this->resource->mw_name))
        throw new \Exception("Can't find middleware to remove in the module");
    }
    ...
    $this->output->info("Middleware '".$this->resource->mw_name."' has been removed from the module '".$this->resource->module_name."' successfully!");
