...
//---------//
  ...
  public function remove() {  try {
    ...
    // 2. Remove artisan command file
    if($fs->exists('warp/modules/'.$this->resource->module_name.'/console/'.$this->resource->cmd_name)) {
      if(!$fs->delete('warp/modules/'.$this->resource->module_name.'/console/'.$this->resource->cmd_name))
        throw new \Exception("Can't find artisan command to remove in the module");
    }
    ...
    $this->output->info("Artisan command '".$this->resource->cmd_name."' has been removed from the module '".$this->resource->module_name."' successfully!");
